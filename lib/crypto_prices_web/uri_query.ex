defmodule CryptoWeb.UriQuery do
  @doc """
  ...
  defp accumulate_kv_pair(prefix, {key, values}, _, acc, opts) when is_map(values) do
    Enum.reduce(values, acc, fn value, acc ->
      accumulate_kv_pair(prefix, {key, value}, true, acc, opts)
    end)
  end

  defp accumulate_kv_pair(prefix, {key, values}, _, acc, [add_indices_to_lists: false] = opts)
